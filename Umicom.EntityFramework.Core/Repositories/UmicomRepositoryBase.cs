...
namespace Umicom.EntityFrameworkCore.Repositories
{
    ...
    public abstract class UmicomRepositoryBase<TEntity, TPrimaryKey> : IRepository<TEntity, TPrimaryKey> where TEntity : Entity<TPrimaryKey>
    {
        //定义数据访问上下文对象
        protected readonly UmicomContext _dbContext;

        /// <summary>
        /// 通过构造函数注入得到数据上下文对象实例
        /// </summary>
        /// <param name="dbContext"></param>
        public UmicomRepositoryBase(UmicomContext dbContext)
        {
    ...
    /// </summary>
    /// <typeparam name="TEntity">实体类型</typeparam>
    public abstract class UmicomRepositoryBase<TEntity> : UmicomRepositoryBase<TEntity, Guid> where TEntity : Entity
    {
        public UmicomRepositoryBase(UmicomContext dbContext) : base(dbContext)
        {
        }
    }
}